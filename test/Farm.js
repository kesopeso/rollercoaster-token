...
contract('Farm', (accounts) => {
    ...
    it('should calculate harvestable reward correctly', async () => {
        ...
        expect(harvestable.gt(ether('24.9999'))).to.be.true;
        expect(harvestable.lt(ether('25.0001'))).to.be.true;

        await farm.stake(ether('1'), { from: curtis });
        await time.increase(time.duration.days(5));
        harvestable = await farm.harvestable(bob);
        expect(harvestable.gt(ether('37.4999'))).to.be.true;
        expect(harvestable.lt(ether('37.5001'))).to.be.true;
        await farm.harvest({ from: bob });

        await time.increase(time.duration.days(4));
        harvestable = await farm.harvestable(bob);
        expect(harvestable.gt(ether('4.9999'))).to.be.true;
        expect(harvestable.lt(ether('5.0001'))).to.be.true;
    });
    ...
    it('should simulate farming successfully', async () => {
        ...
        await time.increase(time.duration.days(4));
        let harvestable = await farm.harvestable(bob);
        expect(harvestable.gt(ether('19.9999'))).to.be.true;
        expect(harvestable.lt(ether('20.0001'))).to.be.true;

        await time.increase(time.duration.days(1));
        await farm.harvest({ from: bob });
        let harvested = await farm.harvested(bob);
        expect(harvested.gt(ether('24.9999'))).to.be.true;
        expect(harvested.lt(ether('25.0001'))).to.be.true;
        harvestable = await farm.harvestable(bob);
        ...
        await time.increase(time.duration.days(1));
        harvestable = await farm.harvestable(bob);
        expect(harvestable.gt(ether('7.4999'))).to.be.true;
        expect(harvestable.lt(ether('7.5001'))).to.be.true;
        claimable = await farm.claimable(bob);
        expect(claimable.gt(ether('4.9999'))).to.be.true;
        expect(claimable.lt(ether('5.0001'))).to.be.true;

        await time.increase(time.duration.days(3));
        harvestable = await farm.harvestable(bob);
        expect(harvestable.gt(ether('13.7499'))).to.be.true;
        expect(harvestable.lt(ether('13.7501'))).to.be.true;
        ...
        expect(harvestable.lt(ether('13.7501'))).to.be.true;
        claimable = await farm.claimable(bob);
        expect(claimable.gt(ether('24.9999'))).to.be.true;
        expect(claimable.lt(ether('25.0001'))).to.be.true;
    });
});
